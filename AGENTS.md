...
| Target | Zweck |
|---|---|
| `make doc-check` | Doku-Links/Anker/Kennungen via `d-check` (Schwester-Tool, digest-gepinnt, netzlos, read-only); seit slice-080 zusätzlich die **Lifecycle-Invariante** ([`SL-002`](docs/plan/planning/observations/README.md)) über `links.resolve-from` — sie hat `verify-slice-links` abgelöst und liegt damit in der Gate- statt der Verifikations-Schicht; seit slice-133 zusätzlich die **Versions-Kohärenz zwischen Dokumenten** über `versions` — eine Prosa-Angabe über die Version eines *anderen* eigenen Dokuments wird gegen dessen Kopf gehalten (`version-stale`); seit slice-173 ein **zweites Muster** für die Baseline-Pins (`.harness/baseline/<tag>/` gegen [`harness/conventions.md`](harness/conventions.md#baseline) §Baseline), das die **Zeitdokument-Klassen** ausnimmt — `harness/conventions/done/`, `docs/plan/planning/done/`, `docs/reviews/` sowie im Beobachtungs-Register die beiden unveränderlichen Rollen `observation.md` und `evidence/**` zitieren den Stand, der damals galt, und werden nicht nachgezogen (`state.md` ist veränderlich und bleibt geprüft). Dass ihre Pfade trotzdem auflösen, trägt die Link-Prüfung, nicht dieses Muster. **Zwei gemessene Grenzen** (slice-173 §2.1): `d-check` nimmt die `current-from`-**Trägerdatei** selbst aus — die 10 Pins in [`conventions.md`](harness/conventions.md) erzeugen kein `version-stale`; gedeckt sind sie nur, weil `.claude/rules/conventions.md` ein Symlink darauf ist und *dieser* Pfad geprüft wird, was ein Nebeneffekt ist und keine Zusage. Und zwei **lebende** Zeiger fallen unter die Verzeichnis-Globs — [`docs/reviews/README.md`](docs/reviews/README.md) (Konvention, kein Report) und der Vorlagen-Link in [`MR-018`](harness/conventions.md#mr-018) **Nicht** Digests: der Erwartungswert kommt versions-förmig aus dem `current-from`-Span, ein `sha256:` bricht dort fail-closed ab — die Digest-Gleichheit bleibt bei `gate-consistency` |
...
| `make symlink-check` | **Zwei** Prüfungen an jedem **getrackten** Symlink (slice-173): das Ziel existiert, und ein Ziel unter `.harness/baseline/` trägt den **adoptierten** Stand. Antwort auf [`BEO-GATE/symlink-ziel-nach-baseline-bump-ungeprueft`](docs/plan/planning/observations/BEO-GATE/symlink-ziel-nach-baseline-bump-ungeprueft/observation.md) bei 3×: `doc-check` liest bei einem Symlink den **Zielinhalt**, nicht den Linkpfad — die vier Verweise unter `.claude/rules/` auf einzelne Regelwerk-Module sah darum kein Gate. Gemessen in slice-173: ein auf einen entfernten Baseline-Stand umgebogener Symlink ließ `doc-check` bei **0** Befunden. Die zweite Prüfung kam aus dem Review desselben Slice — ohne sie wäre slice-167 grün geblieben, wo die Symlinks auf einen **noch vorhandenen** alten Stand zeigten; §Baseline erlaubt zwei Stände während einer Migration, also genau dort. Der adoptierte Stand wird aus [`conventions.md`](harness/conventions.md#baseline) §Baseline gelesen, fail-closed. **Nicht** geprüft: ob ein Symlink *außerhalb* der Baseline auf das inhaltlich richtige Ziel zeigt — das wäre ein Urteil über Absicht |
| `make dcheck-phrase-selftest` | Kalibrierungs-Selbsttest für phrasen-basierte `d-check`-Modul-Konfigurationen (`reviews`-Trigger-Phrase „unabhängiger Review", `structure`s `tasks-ignore-pattern`): vier Kontrollen (2 Muster × Positiv/Negativ) gegen eigene Fixtures und den gepinnten Digest — verhindert das stille Wegdriften einer bereits getroffenen Formulierungs-Wahl. Antwort auf [`BEO-GATE/pruefer-ohne-gegenstand-oder-aufruf`](docs/plan/planning/observations/BEO-GATE/pruefer-ohne-gegenstand-oder-aufruf/observation.md) bei 3× (slice-168). **Nicht** geprüft: ob eine künftig geänderte Formulierung ebenfalls greifen würde — nur die aktuell empfohlene |
