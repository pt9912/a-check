...
| `make symlink-check` | Jeder **getrackte** Symlink löst auf (slice-173). Antwort auf [`BEO-GATE/symlink-ziel-nach-baseline-bump-ungeprueft`](docs/plan/planning/observations/BEO-GATE/symlink-ziel-nach-baseline-bump-ungeprueft/observation.md) bei 3×: `doc-check` liest bei einem Symlink den **Zielinhalt**, nicht den Linkpfad — die vier Verweise unter `.claude/rules/` auf einzelne Regelwerk-Module sah darum kein Gate. Gemessen in slice-173: ein auf einen entfernten Baseline-Stand umgebogener Symlink ließ `doc-check` bei **0** Befunden. **Nicht** geprüft: ob das Ziel das *richtige* ist — nur, dass es existiert; alles andere wäre ein Urteil über Absicht |
