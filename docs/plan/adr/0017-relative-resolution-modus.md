...
das **TypeScript-Backend** (slice-022,
...
6. **Konfigurations-Disziplin:** `mode: relative` nimmt weder `roots` noch `package_base` —
   ...
   [ADR-0016](0016-resolution-sprach-parametrisch.md)/slice-017).
...
- **Erste Nutzerin:** TypeScript (slice-022).
...
| Datum | Ereignis |
|---|---|
| 2026-07-03 | Proposed — Entwurf mit slice-022; Sign-off Auftraggeber ausstehend. |
...
| 2026-07-03 | Proposed → Accepted (Sign-off Auftraggeber: Entscheide A–G gemäß Empfehlung, slice-022 §7). Ab jetzt immutable; Ablösung nur via Folge-ADR mit `Supersedes`. |
