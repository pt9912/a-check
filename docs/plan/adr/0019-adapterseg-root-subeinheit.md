...
1. `adapterSeg` liefert für einen Pfad, dessen Rest nach dem Schicht-Glob-Präfix
   **keinen weiteren `/`** enthält (Datei direkt im Layer-Root), die Sub-Einheit **`''`**.
...
- `make test`: Root↔Root same-layer → kein Befund (die b-cad-Klasse
  ...
  über `resolution`-Kandidaten (`relative`/`fixed-root`).
...
| Datum | Ereignis |
|---|---|
| 2026-07-03 | Proposed — Entwurf mit [slice-024](../planning/open/slice-024-adapterseg-root-subeinheit.md); Evidenz b-cad-Pilot V3b/V4 (39/40 Falsch-Positive); Sign-off Auftraggeber ausstehend. |
