...
**Deckt:** den sechsten Vorfall von `SL-001`, belegt in `1a9f270`.
...
`SL-001` es vorschreibt. Gelesen wurde er nicht, und der Commit folgte
im selben Tool-Call.
...
- `docs/plan/steering-loop.md` — `SL-001` auf sechs Vorfälle.
...
- [x] Der reale Fall aus `1a9f270` liegt als Selbsttest-Fixture vor und ist zusätzlich im echten
      Tool-Call belegt.
- [x] `SL-001` trägt den sechsten Vorfall samt geschlossener Lücke;
...
`SL-001` trägt den sechsten Vorfall samt geschlossener Lücke.
...
`stripQuoted()`. Das ist der **vierte** Vorfall von `SL-004` und der
