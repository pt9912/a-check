...
**Parallel offen — `welle-05-release`:** `v0.1.0` … `v0.6.0` und **`v0.7.0`** sind veröffentlicht
...
`@sha256:41eb368e…` (aktuell v0.7.0) digest-gepinnt in `a-check.mk`); nur die
...
| welle-11-dcheck-pilot-deltas | d-check-Umstellung (Schwester-Repo wartet: die dortige `arch-check`-Ablösung ist per Plan-Review auf drei a-check-Deltas als Vorbedingung gestellt) | **CR im Lastenheft 0.14.0 bindend erfasst** ([AC-FA-RULE-003](../../../../spec/lastenheft.md#ac-fa-rule-003--tech-kapselung-regel-tech-leak)/[AC-FA-CONF-001](../../../../spec/lastenheft.md#ac-fa-conf-001--konfigurationsdatei-a-checkyml), Historie 1/3–3/3): `tech.adapter` auch als Pfad-**Liste** · `composition_root: allow\|forbid` je `tech`-Eintrag · `exclude`-Datei-Globs vor der Extraktion. Umsetzung + Spez-/Handbuch-Nachzug + Release/Digest-Re-Pin: [slice-023](../open/slice-023-dcheck-pilot-deltas.md) (open) | CR erfasst; Umsetzung offen |
