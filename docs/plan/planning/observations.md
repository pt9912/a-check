...
| BEO-008 | `verify-slice-links` nimmt `done/` als Endzustand aus — ein **done**-Slice, der auf einen noch wandernden zeigt, bleibt ungeprüft und bricht, wenn dieser wandert | Planungs-Harness | 2× | slice-093, slice-114 | offen — die Invariante ist auf wandernde Slices gerichtet, nicht auf Verweise **auf** sie; gefangen hat es `doc-check`, nicht der zuständige Sensor |
