...
| BEO-008 | Ein Verweis **auf** einen wandernden Slice bleibt ungeprüft und bricht, wenn dieser wandert — die Lifecycle-Invariante ist auf wandernde Quellen gerichtet, nicht auf Verweise auf sie | Planungs-Harness | 3× | slice-093, slice-114, slice-080 | **Schwelle überschritten** — ab dem dritten Vorfall ist das eine Harness-Lücke und verlangt einen Sensor ([`AGENTS.md`](../../../AGENTS.md) §5). Gefangen hat es jedes Mal `doc-check` **nach** dem `git mv`, nie der zuständige Sensor; `done/` als `fixed-dirs` zu führen ist richtig und deckt genau diesen Fall nicht. Folge-Slice offen |
...
| BEO-022 | Ein CR-Text wird formuliert, ohne ihn durchzurechnen — gegen den **eigenen** Bestand (CR 3: 19 von 19 Abschnitten ausgenommen, danach bleibt keiner) oder gegen den **fremden** Vertrag (CR 4, erster Entwurf: eine nicht-fatale Meldung vorausgesetzt, die `DC-FA-CLI-003` ausdrücklich als Out-of-Scope führt) | Gate-/Werkzeug-Schicht | 2× | slice-080, slice-116 | offen — beide Male war es vorab prüfbar (ein `grep` bzw. ein `sed` über ein öffentliches Lastenheft). Beim dritten Vorfall ist es eine Harness-Lücke und verlangt einen Guide |
