...
nie ein unveröffentlichter Digest). Das Lastenheft steht bei **0.27.0** (eigene Doku-Achse,
...
| 5 | **Qualität** — Gates grün auf **frischem Klon** *und* in der CI mit demselben Image | `make preflight`-Ausgabe (**vor** dem Tag; `ci` **plus** die Range-Schritte des `ci`-Workflows) + CI-Run-Link |
