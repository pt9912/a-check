...
| `make doc-planning` | Lifecycle-Konsistenz Roadmap ↔ `in-progress/` (`d-check`-Modul `planning`): liegt dort ein Slice, benennt ihn die Roadmap-Sektion, statt den Ruhe-Marker zu tragen | `DC-FA-PLAN-001`; slice-122 (konfiguriert und ins Aggregat — davor ohne Gegenstand, [`BEO-014`](../docs/plan/planning/observations.md)) |
...
| `make version-coherence` | Kohärenz doppelt deklarierter Versions-Angaben: ein `uses:`-SHA ⇒ ein Tag-Kommentar; eine Variable in `Makefile` **und** `Dockerfile` ⇒ ein Wert. Prüft **Divergenz**, nicht Wahrheit | slice-131 (Antwort auf [`BEO-026`](../docs/plan/planning/observations.md) bei 3×) |
