...
|---|---|---|
| `make doc-check` | Links/Anker/Kennungs-Linkpflicht/Referenzmatrix der Repo-Doku via `d-check` (Schwester-Tool, digest-gepinnt, `--network none`, read-only) **und** die Lifecycle-Invariante wandernder Slices (`links.resolve-from`) **und** die Versions-Kohärenz zwischen Dokumenten (`versions`: eine Prosa-Angabe über die Version eines anderen eigenen Dokuments gegen dessen Kopf, seit slice-133; seit slice-173 zusätzlich die **Baseline-Pins** gegen [`conventions.md`](conventions.md#baseline) §Baseline, unter Ausnahme der fünf Zeitdokument-Klassen — `conventions/done/`, `planning/done/`, `docs/reviews/` und im Register `observation.md` + `evidence/**`) | Harness-Prozess (Doku-Hygiene; Dogfooding des Stacks); Bootstrap-Gate; [`SL-002`](../docs/plan/planning/observations/README.md) seit slice-080 (löst `verify-slice-links` ab) |
...
| `make doc-structure` | Struktur-Invarianten innerhalb der Dokumente (`d-check`-Modul `structure`): Größen-Regel des DoD, Closure-Struktur, Lerneintrag-Form, Kopffelder, AC-Form — **fünf** Regeln in [`.d-check.yml`](../.d-check.yml), an `verify` gebunden | `DC-FA-STRUCT-001`; slice-115 (Pin), slice-080 (konfiguriert, löst drei Eigenbau-Sensoren ab), slice-120 (`verify-ac-form` als vierter; Pin `v0.69.0` wegen `exempt-expect-count`) |
...
| `make dcheck-phrase-selftest` | Kalibrierungs-Selbsttest für phrasen-basierte `d-check`-Modul-Konfigurationen (`reviews`-Trigger-Phrase, `structure` `tasks-ignore-pattern`): vier Kontrollen (2 Muster × Positiv/Negativ) gegen eigene Fixtures | Harness-Prozess; Antwort auf [`BEO-GATE/pruefer-ohne-gegenstand-oder-aufruf`](../docs/plan/planning/observations/BEO-GATE/pruefer-ohne-gegenstand-oder-aufruf/observation.md) bei 3×; slice-168 |
| `make symlink-check` | **zwei** Prüfungen je getracktem Symlink: Ziel existiert, und ein Baseline-Ziel trägt den adoptierten Stand (`conventions.md` §Baseline, fail-closed gelesen). `doc-check` sieht beides nicht — es liest bei einem Symlink den Zielinhalt, nicht den Linkpfad. Vier Selbsttest-Kontrollen über dieselbe Prüf-Funktion, die das Gate fährt; beide Prüfungen mutations-kalibriert | Harness-Prozess; Antwort auf [`BEO-GATE/symlink-ziel-nach-baseline-bump-ungeprueft`](../docs/plan/planning/observations/BEO-GATE/symlink-ziel-nach-baseline-bump-ungeprueft/observation.md) bei 3×; slice-173 |
...
| `make ci-range-selftest` | Selbsttest der Commit-Range-Weiche der CI (`tools/ci-commit-range.sh`): vier Fälle, darunter der **Force-Push** — eine im Runner-Klon unerreichbare Basis fällt auf den Default-Branch statt abzubrechen | CI-Schicht; slice-134 |
| `make image-scan` | CVE-Scan gegen das **publizierte** Image (Trivy digest-gepinnt, Netz als Zweck): nur **behebbare** CRITICAL/HIGH entscheiden über rot, der Vollbericht fällt nie | [`ADR-0037`](../docs/plan/adr/0037-cve-scan-gegen-das-publizierte-image.md); **nicht** in `gates` (nicht hermetisch); Nachtlauf + `workflow_dispatch`; slice-124 |
| `make doc-planning` | Lifecycle-Konsistenz Roadmap ↔ `in-progress/` (`d-check`-Modul `planning`): liegt dort ein Slice, benennt ihn die Roadmap-Sektion, statt den Ruhe-Marker zu tragen | `DC-FA-PLAN-001`; slice-122 (konfiguriert und ins Aggregat — davor ohne Gegenstand, [`BEO-014`](../docs/plan/planning/observations/BEO-GATE/ruhe-marker-ungewaechtert/observation.md)) |
| `make doc-workflows` | Deklarations-Form der `uses:`-Referenzen unter `.github/workflows` (`d-check`-Modul `workflows`): voller SHA plus Tag-Kommentar beim Fremden, existierendes Ziel und gedeckte Rechte beim Lokalen. Prüft die **Form**, nicht die **Gültigkeit** des Tag-Kommentars | `DC-FA-WF-001`; slice-130 (konfiguriert und ins Aggregat; fand den latenten Release-Bruch in `release.yml`) |
...
| `make gates` | aggregiert die inneren Gates (lint/test/coverage-gate/arch-check/doc-check/doc-targets/doc-planning/doc-workflows/doc-reviews/gate-consistency/version-coherence/suppression-check/symlink-check/dcheck-phrase-selftest/guard-selftest/ci-range-selftest) + `record-gates` als letzter Schritt | — (Aggregat) |
...
| `make verify-observations` | Deckung des Beobachtungs-Registers (Verzeichnisform): zitierter Pfad (neu `BEO-<KUERZEL>/<slug>` oder alt `BEO-NNN` über `Ehemals:`) hat ein Verzeichnis, jedes Verzeichnis trägt ein nicht leeres `evidence/`; Zähler wird abgeleitet, nicht geführt; Lage/Existenz der Beleg-Datei ausdrücklich ungeprüft | Harness-Prozess (Regelwerk `modul-06` §Das Beobachtungs-Register); slice-102, Verzeichnisform slice-139 |
