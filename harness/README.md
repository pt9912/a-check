...
| `make doc-structure` | Struktur-Invarianten innerhalb der Dokumente (`d-check`-Modul `structure`): ohne Konfigurationsblock in [`.d-check.yml`](../.d-check.yml) hat es keinen Gegenstand — als Target deklariert, nicht als Gate gebunden | `DC-FA-STRUCT-001`; slice-115 (Pin `v0.67.0`), Abloesungs-Pruefung slice-080 |
