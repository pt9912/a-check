...
| [`make doc-structure`](sensors/doc-structure.md) | Struktur-Invarianten innerhalb der Dokumente (`d-check`-Modul `structure`): Größen-Regel des DoD, Closure-Struktur, Lerneintrag-Form, Kopffelder, AC-Form — **fünf** Regeln in [`.d-check.yml`](../.d-check.yml), an `verify` gebunden | `DC-FA-STRUCT-001`; slice-115 (Pin), slice-080 (konfiguriert, löst drei Eigenbau-Sensoren ab), slice-120 (`verify-ac-form` als vierter; Pin `v0.69.0` wegen `exempt-expect-count`) |
...
| [`make doc-planning`](sensors/doc-planning.md) | Lifecycle-Konsistenz Roadmap ↔ `in-progress/` (`d-check`-Modul `planning`): liegt dort ein Slice, benennt ihn die Roadmap-Sektion, statt den Ruhe-Marker zu tragen | `DC-FA-PLAN-001`; slice-122 (konfiguriert und ins Aggregat — davor ohne Gegenstand, [`BEO-014`](../docs/plan/planning/observations/BEO-GATE/ruhe-marker-ungewaechtert/observation.md)) |
| [`make doc-workflows`](sensors/doc-workflows.md) | Deklarations-Form der `uses:`-Referenzen unter `.github/workflows` (`d-check`-Modul `workflows`): voller SHA plus Tag-Kommentar beim Fremden, existierendes Ziel und gedeckte Rechte beim Lokalen. Prüft die **Form**, nicht die **Gültigkeit** des Tag-Kommentars | `DC-FA-WF-001`; slice-130 (konfiguriert und ins Aggregat; fand den latenten Release-Bruch in `release.yml`) |
