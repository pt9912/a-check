...
- **Optionalblöcke:** `adapter_sink`, `tech`, `composition_root`, `allow`,
  ...
  - `resolution` → Symbol→Layer-Auflösung **je Sprache** (Map Sprache → `{mode, roots, package_base}`); `mode ∈ {path (Default), fixed-root, relative}`, `namespace` **reserviert** → Exit 2. `fixed-root`: `roots` vorangestellt; **bei gesetztem `package_base`** (gepunktete Sprache) zusätzlich Präfix-Strip + `.`→`/` (eine Pfad-Sprache wie C++ behält ihre `.`-Endungen); greift nur, wenn der Paket-Baum den Verzeichnis-Baum spiegelt ([AC-QA-02](lastenheft.md#ac-qa-02--hermetik-und-ehrliche-heuristik-grenze)). `relative`: Specifier, die `.`/`..` sind oder mit `./`/`../` beginnen, werden lexikalisch gegen das **Verzeichnis der importierenden Datei** normalisiert (`path.Clean`-Semantik); alle anderen Specifier (Bare-Imports) sowie Wurzel-Escapes (führendes `..` **nach** der Normalisierung) liefern eine **leere** Kandidatenmenge — das Roh-Symbol wird nicht als Pfad-Kandidat weitergereicht (kein Geister-Match, [AC-QA-02](lastenheft.md#ac-qa-02--hermetik-und-ehrliche-heuristik-grenze)); `roots`/`package_base` sind bei `relative` unzulässig → Exit 2; Endungs-Agnostik gilt, solange die `layers`-Glob-Präfixe oberhalb der Dateiebene enden (verzeichnisbasierte Globs). **Mehr-Wurzel-Auflösung (datei-mengen-bewusst, fail-closed):** `fixed-root` mit **≥ 2** `roots` löst den internen FQN gegen die **real gescannten Dateien** auf — der Kandidat je Root gilt nur, wenn er einer realen Datei entspricht (endungs-agnostisch, package==directory: ein Wildcard-/Paket-Import — Symbol mit Trailing-Dot — trifft das Paket-Verzeichnis, ein Symbol dessen Datei mit gestrippter Endung oder, bei Klasse≠Datei, sein Paket-Verzeichnis). Die Schicht wird am Pfad des **realen** Kandidaten via [SPEC-RULE-001](#spec-rule-001--regel-auswertung) bestimmt, nicht am Wurzel-Präfix; ein Kandidat ohne reale Datei (Phantom) bleibt **extern**. Löst derselbe FQN real unter ≥ 2 Roots in **verschiedene** Schichten auf (echte Mehrdeutigkeit), bricht `a-check` **nach dem Scan** mit **Exit 2** ab (ein FQN muss in höchstens eine Schicht auflösen); gleiche Schicht (`expect`/`actual`) löst sauber. Grenzen ([AC-QA-02](lastenheft.md#ac-qa-02--hermetik-und-ehrliche-heuristik-grenze)): ein Import ohne reales Paket-Verzeichnis unter einer Root bleibt still extern; verschachtelte-Klassen-Importe und datei-tiefe Globs sind heuristische Grenzen. Fehlt der Block (oder eine Sprache) → Import-als-Pfad. Nutzt Sprache **und Pfad** der Quelldatei ([SPEC-RULE-001](#spec-rule-001--regel-auswertung)/[SPEC-EXTRACT-001](#spec-extract-001--import-extraktion)).
