...
| 0.16.0 | 2026-07-04 | `SPEC-CONF-001`: **Mehr-Wurzel-Guard (fail-closed)** — `mode: fixed-root` mit ≥ 2 `roots`, von denen ≥ 2 je vollständig in einer (verschiedenen) Schicht enthalten sind (`layers`-Glob-Präfix Vorfahr-oder-gleich des Roots), bricht mit Exit 2 statt still Phantom-Kandidaten fehlzuklassifizieren. Folgt [`AC-FA-CONF-001`](lastenheft.md#ac-fa-conf-001--konfigurationsdatei-a-checkyml) 0.16.0. |
