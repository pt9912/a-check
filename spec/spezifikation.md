...
| 0.32.0 | 2026-09-19 | `SPEC-RULE-001`: **`portScope` überspringt ein abschließendes Richtungssegment** — die Ableitung nahm das längste `port`-Glob-Literalpräfix **minus seinem letzten Segment** („Port-Ordner-Marker, typisch `ports`"). Endet der Glob auf dem **Richtungssegment** (`…/ports/outbound/**`), war der Marker das Richtungssegment selbst: Der Scope fiel auf `…/ports`, `appTreeContains` wurde false, und `port-locality` **schwieg** — kein Befund, Exit 0, bei einem echten Verstoß. Damit schaltete genau die Konfiguration, die `direction` an einer Port-Schicht deklariert und so `port-direction-mismatch` scharf macht, die **andere** kategorische Regel still; gemessen an einem Konsumenten, der den Umbau nach `ports/inbound|outbound` vollzogen hatte. Die Ableitung zieht jetzt zusätzlich das Richtungssegment ab, wenn es dem an **dieser** Schicht deklarierten `direction`-Wert entspricht — der Ordner-Name wird nicht geraten. **Ergänzend** eine dritte Advisory-Diagnose ([SPEC-CLI-001](#spec-cli-001--aufruf-scan-wurzel-und-exit-codes)) für die Restfälle, in denen die Ableitung den Port-Baum weiterhin nicht erreicht. Kein Lastenheft-Bump: [AC-FA-RULE-010](lastenheft.md#ac-fa-rule-010--port-lokalität-regel-port-locality) ist unverändert, nur seine Ableitung war unterbestimmt ([ADR-0040](../docs/plan/adr/0040-portscope-richtungssegment.md)). slice-194. |
