...
- **Schicht-Rollen** ([AC-FA-RULE-006](lastenheft.md#ac-fa-rule-006--schicht-rollen-generische-regel-anwendung)): ein `layers`-Eintrag ist **entweder** eine Glob-Liste (`name: [globs]`) **oder** ein Objekt `{globs: [...], role: domain|app|port|adapter, direction: ...}` (`direction` optional). Fehlt `role`, wird es aus konventionellen Namen abgeleitet (`core`→`domain`, `ports`→`port`, `adapters`→`adapter`, `application`/`app`→`app`); `role:` hat Vorrang. Die Reinheits-Regeln (`core-impurity`/`app-impurity`/`port-impurity`/`lateral-adapter`) greifen über die Rolle, nicht den Namen — fremd benannte Schichten sind damit voll prüfbar. Optional trägt eine `port`-/`adapter`-Schicht zusätzlich eine **Richtung**, deren Wertebereich **rollen-abhängig** ist ([AC-FA-RULE-008](lastenheft.md#ac-fa-rule-008--richtungs-dimension-regel-port-direction-mismatch), **orthogonal** zur Rolle): an `role: port` `inbound|outbound`, an `role: adapter` `driving|driven`. **Jede andere Kombination ist ein Konfigurationsfehler** (Exit 2); die Meldung nennt Schicht, Wert und die für **diese Rolle** gültige Menge — eine feste Menge zu nennen schickte den Leser auf die falsche Hälfte. Die Connectivity-Regel `port-direction-mismatch` prüft, dass ein Adapter nur Ports **seiner** Seite importiert — ohne `direction` keine Prüfung.
...
| `port-direction-mismatch` | Datei mit Rolle `adapter` und Richtung X importiert eine `port`-Rolle mit Richtung Y, wobei X und Y **nicht das Paar bilden** (`driving`↔`inbound`, `driven`↔`outbound`; **beide gesetzt**) — ein treibender Adapter spricht nur eingehende Ports, ein getriebener nur ausgehende. Geprüft wird eine **Paarung**, keine String-Gleichheit: die beiden Seiten führen verschiedene Vokabulare ([AC-FA-RULE-008](lastenheft.md#ac-fa-rule-008--richtungs-dimension-regel-port-direction-mismatch)). **Orthogonal** zur Rolle, ohne `direction` keine Prüfung. **Kategorisch** (nicht über `edges`/`allow` aufhebbar, wie `lateral-adapter`) | [AC-FA-RULE-008](lastenheft.md#ac-fa-rule-008--richtungs-dimension-regel-port-direction-mismatch) |
| `port-locality` | Datei mit Rolle `app` importiert ein **im Application-Baum geschachteltes** `port`-Ziel, dessen **Scope-Verzeichnis** ihren Pfad nicht enthält — `portScope ≠ "" ∧ appTreeContains(portScope) ∧ segIndex(quelle, portScope) < 0`, wobei `portScope(k)` = das **längste `port`-Rollen-Glob-Literalpräfix**, das `k` matcht, **minus seinem letzten Pfad-Segment** (Port-Ordner-Marker, typisch `ports`); endet das Glob auf einem Segment, das dem an **dieser** Schicht deklarierten `direction`-Wert entspricht, fällt auch dieses — das Richtungssegment liegt **unter** dem Port-Ordner (`…/ports/outbound/**`), nicht darüber —, **sofern** der Port im App-Baum liegt und der Scope durch den zusätzlichen Schnitt in ihn **gelangt**, statt über ihn hinauszugehen: use-case-lokal (`…/createorder`) ⊂ business-area (`…/order`) ⊂ app-weit (`…/application`). `appTreeContains(s)` = `s` ist Vorfahr eines `app`-Glob-Präfixes (der Port liegt *im* App-Baum). **Geschwister-Ports** (klassisch, `…/ports` neben `…/services`) → `appTreeContains` false → **inert**. **Nur `app`-Importeure** (ein Adapter, der einen Port implementiert, ist nicht erfasst). **Kategorisch** (nicht über `edges`/`allow` aufhebbar) | [AC-FA-RULE-010](lastenheft.md#ac-fa-rule-010--port-lokalität-regel-port-locality) |
...
(Port-Ordner-Marker) und, wenn das Glob auf dem `direction`-Wert **dieser** `port`-Schicht endet,
**minus diesem Richtungssegment zusätzlich** — ein Richtungssegment unter dem Port-Ordner
(`…/ports/outbound/**`) darf eine andere kategorische Regel nicht abschalten
([AC-FA-RULE-008](lastenheft.md#ac-fa-rule-008--richtungs-dimension-regel-port-direction-mismatch)) —
jedoch nur dort, wo der Port im App-Baum liegt und der Schnitt den Scope **in** ihn bringt, statt
über ihn hinaus: Der Schnitt darf den Scope nie weiter machen als der Marker-Schnitt allein. Beide vergleichen **kanonisch** (das Glob-Präfix ist für Quelle wie Kandidat dieselbe
...
- **Auflösungs-Diagnose** (advisory, auf stderr **nach** der Grenz-Diagnose): Löst im **gesamten
  ...
  **null** Befunden.
- **Port-Scope-Diagnose** (advisory, auf stderr **nach** der Auflösungs-Diagnose): Nennt je `port`-
  Glob eine Zeile `<glob> -> Scope <verzeichnis>`, wenn das **Verzeichnis** des Globs im App-Baum
  liegt, der daraus abgeleitete **Scope** ihn aber **nicht** erreicht — die Konfigurationsform, in
  der [`port-locality`](#spec-rule-001--regel-auswertung) still bleibt, obwohl Lokalität für diesen
  Port sinnvoll wäre. Das Kriterium ist der **Defekt, nicht seine Ursache**: Ein Glob auf einem
  Richtungssegment ist der gemessene Anlass; ein Tippfehler oder eine anders benannte
  Richtungsebene sehen von hier aus gleich aus.

  **Geschwister-Ports werden nicht gemeldet** — ihre Verzeichnisse liegen schon außerhalb des
  App-Baums, und ihre Inertheit ist
  [`AC-FA-RULE-010`](lastenheft.md#ac-fa-rule-010--port-lokalität-regel-port-locality) ausdrücklich
  zugesagt; eine Meldung dort wäre auf jedem klassisch-hexagonalen Repo Rauschen und würde
  abgeschaltet statt befolgt. Geprüft wird die **Konfiguration**, nicht ein Import: ein Baum ohne
  Import auf einen solchen Port hat dieselbe Lücke, wartend auf den ersten. Stabil nach Glob
  sortiert ([SPEC-DET-001](#spec-det-001--determinismus-vertrag)); **Exit-Code unberührt**, Meldung
  auch bei **null** Befunden, ab zehn Globs gekürzt **mit Restzahl**.
...
| 0.32.0 | 2026-09-19 | `SPEC-RULE-001`: **`portScope` überspringt ein abschließendes Richtungssegment** — die Ableitung nahm das längste `port`-Glob-Literalpräfix **minus seinem letzten Segment** („Port-Ordner-Marker, typisch `ports`"). Endet der Glob auf dem **Richtungssegment** (`…/ports/outbound/**`), war der Marker das Richtungssegment selbst: Der Scope fiel auf `…/ports`, `appTreeContains` wurde false, und `port-locality` **schwieg** — kein Befund, Exit 0, bei einem echten Verstoß. Damit schaltete genau die Konfiguration, die `direction` an einer Port-Schicht deklariert und so `port-direction-mismatch` scharf macht, die **andere** kategorische Regel still; gemessen an einem Konsumenten, der den Umbau nach `ports/inbound|outbound` vollzogen hatte. Die Ableitung zieht jetzt zusätzlich das Richtungssegment ab, wenn es dem an **dieser** Schicht deklarierten `direction`-Wert entspricht — der Ordner-Name wird nicht geraten. **Ergänzend** eine weitere Advisory-Diagnose ([SPEC-CLI-001](#spec-cli-001--aufruf-scan-wurzel-und-exit-codes)) für die Restfälle, in denen die Ableitung den Port-Baum weiterhin nicht erreicht. Kein Lastenheft-Bump: [AC-FA-RULE-010](lastenheft.md#ac-fa-rule-010--port-lokalität-regel-port-locality) ist unverändert, nur seine Ableitung war unterbestimmt ([ADR-0040](../docs/plan/adr/0040-portscope-richtungssegment.md)). slice-194. |
